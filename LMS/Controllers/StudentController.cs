    ...
    public class StudentController : CommonController
    {
        ...
        public IActionResult GetMyClasses(string uid)
        {
            using (db)
            {
                var result =
                    ...
                    into enrollClass
                    from x in enrollClass
                    join cour in db.Courses
                    on x.Course equals cour.CourseId
                    select new
                    {
        ...
        public IActionResult GetAssignmentsInClass(string subject, int num, string season, int year, string uid)
        {
            using (db)
            {
                ...
                select new {id = ass.AssignId, aname = ass.Name, cname = cata.Name, due = ass.Due};
        ...
        public IActionResult Enroll(string subject, int num, string season, int year, string uid)
        {
            using (db)
            {
                try
                {
                    ...
                    join cla in db.Classes
                    on cour.CourseId equals cla.Course
                    where cla.Semester == season && cla.Year == year
                    select cla.ClassId;

                    uint toEnroll = result.Single();

                    Enrollment enroll = new Enrollment();
                    enroll.Class = toEnroll;
                    enroll.Student = uid;
